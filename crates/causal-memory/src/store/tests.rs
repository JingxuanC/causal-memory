...
fn test_generated_ids_survive_process_restart() {
    ...
    let item = |seq: usize| crate::distill::MemoryItem {
...
fn test_open_enables_wal_and_busy_timeout() {
    ...
    mem.record_decision("d", "o", "caused", Some("t"), 0.5, "rule")
        .unwrap();
}
...
fn test_find_falsified_candidates() {
    ...
    let pairs: Vec<(&str, &str)> = cands
        .iter()
        .map(|(_, _, _, o, _, n)| (o.as_str(), n.as_str()))
        .collect();
    assert!(
        pairs
            .iter()
            .any(|(o, n)| o.contains("deadlock") && n.contains("worked fine")),
        "(failed -> worked) pair present, got {pairs:?}"
    );
    assert!(
        pairs
            .iter()
            .any(|(o, n)| o.contains("worked fine") && n.contains("deadlock")),
...
fn test_cooccurrence_learning_persists() {
    ...
    let d1: String = store
        .with_conn(|c| {
            Ok(c.query_row(
                "SELECT id FROM chunks WHERE text = ?1",
                rusqlite::params!["used Redis cache"],
                |r| r.get(0),
            )?)
        })
        .unwrap();
    let d2: String = store
        .with_conn(|c| {
            Ok(c.query_row(
                "SELECT id FROM chunks WHERE text = ?1",
                rusqlite::params!["added TTL"],
                |r| r.get(0),
            )?)
        })
        .unwrap();
    store
        .bump_cooccurrences(&[(d1.clone(), d2.clone())])
        .unwrap();
    store
        .bump_cooccurrences(&[(d1.clone(), d2.clone())])
        .unwrap();
    ...
    assert!(
        found_cooc,
        "reopened store loads the learned co-occurrence edge"
    );
}
...
fn test_trace_cause_cross_session_basic() {
    ...
    let _e_api_deep = link_tagged(&store, "api deeper cause", "api root", "caused", 0.9, "api");
    ...
    assert_eq!(cross.segments[1].hops[0].decision_text, "auth deeper cause");
...
fn test_trace_cause_cross_session_single_session_fallback() {
    ...
    assert_eq!(
        results.len(),
        1,
        "expected exactly one single-session chain"
    );
...
fn test_retire_facts_by_hint_excludes_new_fact() {
    ...
    let _old_id = store
        .record_fact(
            "preference",
            "user prefers almond milk in their coffee",
            "user",
            "distill",
            0.8,
        )
    ...
    let new_id = store
        ...
        .unwrap();
    let retired = store
        .retire_facts_by_hint("preference", "user", "almond milk", Some(new_id))
        .unwrap();
    ...
    assert_eq!(
        retired_self, 1,
        "without the exclusion the new fact retires itself"
    );
    let hits = store.search_facts_bm25("oat milk", None, 10).unwrap();
...
fn test_annotate_superseded_is_soft() {
    ...
    let old_id = all
        .iter()
        .find(|e| e.decision_text.contains("FixOne"))
        .unwrap()
        .edge_id;
    let new_id = all
        .iter()
        .find(|e| e.decision_text.contains("FixTwo"))
        .unwrap()
        .edge_id;
...
fn test_entity_boost_amplifies_semantic_scores() {
    ...
    for e in &edges {
        store
            .put_embedding(e.edge_id, "test", &[1.0f32, 0.0, 0.0, 0.0])
            .unwrap();
    }
...
fn test_search_causal_hop_expands_chain() {
    ...
    let id_a = hop_link(
        &store,
        "went camping with Melanie at Yosemite",
        "Melanie loved the trip",
        0.9,
    );
    hop_link(
        &store,
        "Melanie loved the trip",
        "booked another camping trip",
        0.8,
    );
    ...
    assert!(hop
        .iter()
        .any(|e| e.outcome_text.contains("booked another")));
...
fn test_record_decision_reuses_identical_text_chunk() {
    ...
    let code: String = store
        .with_conn(|c| {
            c.query_row(
                "SELECT sparse_code FROM chunks WHERE id = ?1",
                params![id1],
                |r| r.get(0),
            )
...
fn test_bm25_seed_ids_spans_both_namespaces_and_filters_scope() {
    ...
    let scoped = store
        .bm25_seed_ids("TypeScript module", Some("agent"), 10)
        .unwrap();
    assert!(
...
fn test_search_causal_bm25_oversized_index_candidates_fall_back() {
    ...
    let hits = store
        .search_causal_bm25(Some("bigtag"), "shared token", 5)
        .unwrap();
    assert!(
        !hits.is_empty(),
        "oversized candidates must fall back to the scan"
    );
    assert!(
