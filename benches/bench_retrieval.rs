...
fn main() -> anyhow::Result<()> {
    ...
    let probes = vec![
        (
            "Q1",
            "What did we decide about entropy framing?",
            "reasoning",
            "Shannon",
        ),
        ...
        (
            "Q3",
            "What's the relationship between compaction and causal info?",
            "reasoning",
            "compaction",
        ),
        (
            "Q4",
            "What was the decision about multi-scale memory?",
            "reasoning",
            "多尺度",
        ),
        (
            "Q5",
            "What did we learn about LLM statelessness?",
            "reasoning",
            "无状态",
        ),
        (
            "Q6",
            "Why is causal memory important?",
            "reasoning",
            "causal",
        ),
        (
            "Q7",
            "What was decided about the compaction experiment?",
            "reasoning",
            "实验",
        ),
        (
            "Q8",
            "What's the key insight about long context windows?",
            "reasoning",
            "上下文",
        ),
        (
            "Q9",
            "How did we handle the rebuttal about creativity?",
            "reasoning",
            "反驳",
        ),
        (
            "Q10",
            "What was the Letta roadmap comparison?",
            "reasoning",
            "Letta",
        ),
    ];
    ...
    println!(
        "Testing {} probe questions against {} edges\n",
        probes.len(),
        edge_count
    );
    ...
    for (qid, question, expected_tag, keyword) in &probes {
        ...
        let tag_results = store
            .search_causal(Some(expected_tag), None)
            .unwrap_or_default();
        ...
        let causal_results = store
            .search_causal(Some(expected_tag), Some(keyword))
            .unwrap_or_default();
        ...
        println!(
            ...
            if kw_relevant { "✅" } else { "❌" },
            kw_results.len(),
            if tag_relevant { "✅" } else { "❌" },
            tag_results.len(),
            if causal_relevant { "✅" } else { "❌" },
            causal_results.len(),
        );
    }
    ...
    println!("|---|---|");
    println!(
        "| Keyword LIKE (basic RAG)  | {}/{} ({:.0}%) |",
        kw_hits,
        probes.len(),
        kw_hits as f64 / probes.len() as f64 * 100.0
    );
    println!(
        "| Task-tag filter           | {}/{} ({:.0}%) |",
        tag_hits,
        probes.len(),
        tag_hits as f64 / probes.len() as f64 * 100.0
    );
    println!(
        "| Causal (task + keyword)   | {}/{} ({:.0}%) |",
        causal_hits,
        probes.len(),
        causal_hits as f64 / probes.len() as f64 * 100.0
    );

    // Also test multi-hop trace capability (unique to causal-memory)
    println!("\n=== Multi-hop trace test (unique to causal-memory) ===");
    let chains = store
        .trace_cause_chain("memory", 5, 0.3)
        .unwrap_or_default();
    if chains.is_empty() {
        ...
        for (i, chain) in chains.iter().take(3).enumerate() {
            ...
            for hop in chain {
                println!(
                    "    hop {}: {}",
                    hop.hop,
                    &hop.decision_text[..hop.decision_text.len().min(60)]
                );
            }
        }
    }
