...
Long-running LLM agents that operate across hundreds of tasks and iterative context compressions lose causal information — the knowledge of *why* a decision was made and *what* it led to — faster than any other memory type. We present **causal-memory**, an agent memory system that models all memory types (facts, temporal state, causal edges, co-occurrence patterns) as typed edges on a single graph, processed by a hippocampus-inspired engine with excitatory and inhibitory spreading activation. The system's core innovation is the **excitatory/inhibitory duality**: `caused` edges spread positive activation (a glutamate analogue) while `prevented` edges spread negative activation (a GABA analogue) — no existing memory system implements inhibitory dynamics. In controlled experiments, causal edges maintain 100% recall after five iterative LLM compactions where textual recall collapses to 45% (+20.8pp rescue in combined QA). On the LoCoMo benchmark (1,986 questions), the system achieves 79.1% accuracy under strict judging and 84.1% under a mem0-compatible judge, narrowing the gap to mem0's published 91.6% to 2–3 percentage points attributable to model quality. An end-to-end agent ablation shows that causal memory reduces the repeat-mistake rate from 67% to 33% on known-trap tasks. The causal graph is designed to function as an explicit world model — backward traversal performs attribution (benchmarked), while forward traversal via `intervention_query` performs simulation (implemented but not yet benchmarked for prediction accuracy). causal-memory is open-source (Rust, Apache-2.0) with 170 tests and reproducible benchmark harnesses.
...
The problem is acute because **causal information is the most fragile memory type under iterative context compaction**. When an agent's conversation history exceeds its context window, the framework summarizes older messages — a lossy operation repeated hundreds of times in continuous operation. Each summarization pass may preserve different aspects of the source, causing progressive semantic drift. We measure this directly: after five compaction passes with a production LLM summarization prompt, textual recall of causal details (which decision led to which outcome) drops to **45%**, while the same information stored as typed causal edges in an external graph maintains **100%** recall. The degradation is not gradual but a **sudden-death cliff** between the second and third compaction (85% → 55%). We note that any external storage would survive compaction; the contribution of the causal-graph architecture is not survival itself but **retrieval quality** — the typed-edge structure enables polarity-aware spreading activation and causal chain traversal that a flat key-value store cannot offer (demonstrated by the LoCoMo, LongMemEval, and agent ablation results).
...
3. **The causal graph is designed as an explicit world model.** A `caused` edge `[decision A] → [outcome B]` is a sample of the transition function *f(state, action) → outcome*. Backward traversal (outcome → decision) performs **attribution** — all existing benchmarks measure this. Forward traversal (decision → predicted outcomes) is designed to perform **simulation**, and the `intervention_query` tool implements this capability. However, forward simulation is currently **unbenchmarked**: no dedicated experiment measures its prediction accuracy. We position it as an implemented feature with future-work validation, not a validated contribution.

We evaluate causal-memory across five dimensions. **Compaction survival**: causal edges maintain 100% recall after 5 compactions, rescuing combined QA accuracy by +20.8pp. **LoCoMo benchmark**: 79.1% overall (strict judge) / 84.1% (mem0-compatible judge) via a six-configuration optimization matrix isolating prompt engineering (+4.6pp), retrieval budget (+3.8pp), and semantic retrieval (+1.1pp). The gap to mem0's 91.6% is ~7.5pp, largely attributable to answerer model quality. **LongMemEval**: multi-session accuracy improves from 41.4% to 57.9% through iterative query expansion and session-level context widening. **Agent ablation**: repeat-mistake rate on known traps drops from 67% to 33%. **Inhibitory ablation**: prevented edges produce 10/10 negative-activation warning signals (0/10 when disabled) without degrading precision. **Model sensitivity**: a reasoning model (deepseek-v4-pro) achieves 82.3% per-question accuracy, confirming the architecture gap is a model gap.

**Contributions.** (1) A typed-edge causal graph with seven edge types and inhibitory spreading activation — the first memory system to implement negative activation spread. (2) A compaction-survival benchmark demonstrating that externalized causal storage maintains 100% recall where textual recall collapses to 45%; the graph architecture's advantage over flat storage is validated by retrieval-quality results on LoCoMo, LongMemEval, and agent tasks. (3) An end-to-end demonstration that causal memory reduces repeat-mistake rates in trap-avoidance tasks. (4) A controlled ablation isolating the contribution of inhibitory (prevented) edges: they produce negative-activation warning signals that are absent when disabled, without degrading retrieval precision. (5) A reproducible, open-source system (Rust, 13 MCP tools, 170 tests, four benchmark harnesses). Forward simulation via `intervention_query` is implemented but not yet benchmarked for prediction accuracy; it is positioned as future work rather than a validated contribution.
...
The causal graph is designed as an explicit world model in the sense of Physical Intelligence's definition (arXiv:2607.06401): a compression of the state-transition process under finite resources. Each `caused` edge is a sample of the transition function *f*(*s*, *a*) → *s*′. Backward traversal (trace_cause) performs attribution — answering "which decision led to this outcome?" — and is validated on all benchmarks. Forward traversal (intervention_query) is designed to perform simulation — answering "what will happen if I make this decision?" — but is not yet validated by a dedicated prediction-accuracy benchmark.

This positions causal-memory beyond the "notebook" category occupied by all current memory systems. A notebook records what happened; a causal graph encodes why it happened and what it led to. Forward simulation is the natural next step — a pure fact store cannot simulate because facts lack directionality (knowing "the user uses Redis" does not predict "using Redis will cause cache stampede"). The causal graph's typed edges provide the directionality and polarity needed for forward rollout. However, validating this capability requires prediction-accuracy benchmarks that do not yet exist; we leave this as our highest-priority future work.
...
Agent memory should be causal, not just factual. We have shown that causal edges — typed relationships between decisions and their outcomes — survive iterative context compaction where all text-based memory collapses, that they reduce repeat-mistake rates in end-to-end agent tasks, and that inhibitory (prevented) edges produce unique warning signals absent when disabled. The excitatory/inhibitory duality, absent from all competitors including HeLa-Mem (ACL 2026), is the core architectural insight: a complete memory system needs both "what caused this" and "what prevents this," just as the biological hippocampus needs both glutamate and GABA.

The path forward is not higher benchmark scores on existing QA tasks — it is the design of benchmarks that measure what causal memory uniquely offers: compaction survival, forward simulation accuracy (currently unbenchmarked), and cross-session causal attribution. The causal graph as an explicit world model — with validated backward attribution and implemented-but-unvalidated forward simulation — is the framing that connects agent memory to the broader goal of systems that learn from experience and predict the consequences of their actions.
