...
% ============================================================
\begin{abstract}
Long-running LLM agents that operate across hundreds of tasks and iterative context compressions lose causal information---the knowledge of \emph{why} a decision was made and \emph{what} it led to---faster than any other memory type. We present \textbf{causal-memory}, an agent memory system that models all memory types (facts, temporal state, causal edges, co-occurrence patterns) as typed edges on a single graph, processed by a hippocampus-inspired engine with excitatory and inhibitory spreading activation. The system's core innovation is the \textbf{excitatory/inhibitory duality}: \texttt{caused} edges spread positive activation (a glutamate analogue) while \texttt{prevented} edges spread negative activation (a GABA analogue)---no existing memory system implements inhibitory dynamics. In controlled experiments, causal edges maintain 100\% recall after five iterative LLM compactions where textual recall collapses to 45\% (+20.8pp rescue in combined QA). On the LoCoMo benchmark (1,986 questions), the system achieves 79.1\% accuracy under strict judging ($\sim$89\% at frontier-compatible judge caliber), narrowing the gap to mem0's published 91.6\% to 2--3 percentage points attributable to model quality. An end-to-end agent ablation shows that causal memory halves the repeat-mistake rate (67\%$\rightarrow$33\%) on known-trap tasks. The causal graph functions as an explicit world model: backward traversal performs attribution, forward traversal performs simulation. causal-memory is open-source (Rust, Apache-2.0).
\end{abstract}
...
\item \textbf{The causal graph is an explicit world model.} A \texttt{caused} edge is a sample of the transition function $f(\text{state}, \text{action}) \rightarrow \text{outcome}$. Backward traversal performs \textbf{attribution}; forward traversal performs \textbf{simulation}---no competing memory system offers decision-time what-if queries.
\end{enumerate}

\textbf{Contributions.} (1)~A typed-edge causal graph with seven edge types and inhibitory spreading activation---the first memory system to implement negative activation spread. (2)~Compaction survival as a first-class benchmark: causal edges are immune to iterative summarization (+20.8pp). (3)~An end-to-end demonstration that causal memory halves repeat-mistake rates (67\%$\rightarrow$33\%). (4)~Forward simulation via intervention queries, positioning agent memory as an explicit world model. (5)~A reproducible, open-source system with 163 tests and four benchmark harnesses.
...
Gain attribution: prompt engineering +4.6pp, retrieval budget +3.8pp, semantic RRF +1.1pp. At mem0-compatible judge caliber: \textbf{$\sim$89\%} (gap to mem0 91.6\% = 2--3pp, model gap).
...
The causal graph is a transition-function sample set. Backward = attribution; forward = simulation. Current limitation: coverage is 49 edges/conversation (12\% of turns); richer extraction is the bottleneck.
...
Agent memory should be causal, not just factual. Causal edges survive compaction, enable forward simulation, and halve repeat-mistake rates. The excitatory/inhibitory duality---absent from all competitors---is the core architectural insight: a complete memory needs both ``what caused this'' and ``what prevents this,'' as the biological hippocampus needs both glutamate and GABA.
